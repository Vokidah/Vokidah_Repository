                        ...
                        if (URI.contains(path)) {
                            if (reqMethod.equals(RequestType.GET.value())) {
                                ...
                                if(URI.equals(path) || URI.equals(path+"/")) {
                                    method = getMethod(handlerClass, path, RequestType.GET, "");
                                    System.out.println(method);
                                    result = method.invoke(handler, null);
                                }
                                else{
                                    method = getMethod(handlerClass, path, RequestType.GET, "/:id");
                                    if(URI.contains(path+"/")) {
                                        String id = URI.substring(path.length() + 1, URI.length());
                                        if(id.substring(id.length()-1,id.length()).equals("/"))
                                            id=id.substring(0,id.length()-1);
                                        result = method.invoke(handler, id);
                                    }
                                }
                                Gson gson = new Gson();
            ...
            for (Annotation annotation : methodAnnotations) {
                if (requestType.equals(RequestType.GET)) {
                    if (annotation instanceof Get) {
                        boolean isEmpty = ((Get) annotation).value().equals(type);
                        if (isEmpty)
